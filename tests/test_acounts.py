...
def test_create_account_duplicate_number(client, existing_client, admin_headers):
    ...
    client.post("/api/v1/accounts", params={"client_id": existing_client["id"]}, json=payload, headers=admin_headers)
    response = client.post("/api/v1/accounts", params={"client_id": existing_client["id"]}, json=payload, headers=admin_headers)
...
def test_get_client_with_accounts(client, existing_client,admin_headers):
    ...
    response = client.get(f"/api/v1/clients/{existing_client['id']}/accounts",headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
